...
The Work-Efficient Scan is clearly faster than the Naive Scan since there is less overall work to do as described in the implementation details in the [Scan](#Scan) section.
...
Lastly, since the Naive & Hardware-Efficient and Work-Efficient & Hardware-Efficient implementations are optimized with shared memory and warp paritioning best practices, another bottleneck comes into play called bakn conflicts. Shared memory is split up to 32 banks such that each bank can service on address per cycle. This was not a problem when implementing the Naive & Hardware-Efficient Scan, but was introduced in the initial implementations of the Work-Efficient & Hardware-Efficient Scan. Through adding a padding offset to after every 32 shared memory elements, the bank conflicts were alleviated and performance increased.
